...
def app():
    ...
    average_or_not = st.checkbox(f"Average Maximum Tanimoto similarity from Top-{top_n}", value=False)
    ...
    plot_molecule_size_vs_performance = st.checkbox("Plot Molecule Size vs Performance", value=False)
    ...
    if average_or_not:
        st.subheader(f"Max Tanimoto Similarity of Top-{top_n}")
        st.write(mean.T.sort_values(by=f"Top {top_n}", axis=1, ascending=False).style.highlight_max(axis=1))
