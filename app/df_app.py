...
def save_molecule_image(smiles, file_path):
    ...
    img = Draw.MolToImage(mol, width=900)
    img.save(file_path)
    return file_path
...
if uploaded_file is not None:
    ...
    with contextlib.suppress(Exception):
        ...
        filtered_df = df_filtered.sort_values(by=similarity_metric, ascending=False).head(top_n)

        # Find the molecule with Tanimoto similarity of 1
        tanimoto_1_df = df_filtered[df_filtered["Tanimoto Similarity"] == 1]
    ...
    with contextlib.suppress(Exception):
        if not tanimoto_1_df.empty:
            ...
            st.subheader("No molecule with Tanimoto Similarity of 1 found in the dataset.")
    ...
    with contextlib.suppress(Exception):
        ...
        for (index, row), col in zip(filtered_df.iterrows(), column_list):
            with col:
                ...
                save_molecule_image(row["SMILES"], image_path)
                st.image(image_path, caption=f"{similarity_metric}: {row[similarity_metric]:.2f}")
