...
from secs.models import MolBind, available_encoders, register_encoder, resolve_encoder
...
from secs.models.registry import DEFAULT_BACKBONE
...
def test_resolve_picks_the_named_backbone(registered_stub, stub_encoder_cls):
    assert resolve_encoder("smiles", registered_stub) is stub_encoder_cls
...
def test_molbind_builds_the_backbone_named_in_the_config(registered_stub, secs_config, stub_encoder_cls):
    model = MolBind(secs_config(registered_stub, "transformer"))
    assert isinstance(model.dict_encoders["smiles"], stub_encoder_cls)
...
def test_molbind_defaults_the_backbone_when_the_config_omits_a_name(registered_stub, secs_config):
    model = MolBind(secs_config(registered_stub, None))
    assert isinstance(model.dict_encoders["c_nmr"], CNmrTransformerEncoder)


def test_molbind_forward_returns_one_embedding_per_modality(registered_stub, secs_config):
    model = MolBind(secs_config(registered_stub, "transformer")).eval()
    batch = {
...
def test_molbind_rejects_an_unsupported_modality(registered_stub, secs_config):
    cfg = secs_config(registered_stub, "transformer")
