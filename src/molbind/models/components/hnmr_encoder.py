...
# Simpler scaling function for common use cases
def create_scaled_encoder(
    scale: float = 1.0, input_length: int = 10000, latent_dim: int = 1024, use_attention: bool = True, **kwargs
):
    """
    Simple scaling function where scale controls overall model size

    Args:
        scale: Overall scaling factor (0.5 = smaller, 2.0 = larger)
    ...
    if scale <= 0.5:
        config = {
            ...
            "dropout": 0.05,
            "use_attention": use_attention and scale >= 0.5,
            "attention_heads": 4,
            "use_se": False,
        }
    elif scale <= 1.0:
        ...
        config = {
            "width_mult": scale * 0.8,  # Slightly less width scaling
            "depth_mult": scale,
        ...
        config = {
            "width_mult": scale * 0.7,
            "depth_mult": scale * 0.8,
            "base_channels": [128, 256, 512, 1024, 2048],
