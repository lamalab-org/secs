    ...
    def retrieval_metrics(
        ...
        if self.world_size > 1:
            ...
            all_embeddings_central_mod = self.all_gather(
                embeddings_central_mod, sync_grads=False
            )
            all_embeddings_other_mod = self.all_gather(
                embeddings_other_mod, sync_grads=False
            )
...
class MolBindModuleCustomSamples(MolBindModule):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.loss = InfoNCE(
            temperature=self.config.model.loss.temperature, negative_mode="paired"
        )
        self.loss_unpaired = InfoNCE(
            temperature=self.config.model.loss.temperature, negative_mode="unpaired"
        )
    ...
    def _info_nce_loss(
        self, z1: Tensor, z2: Tensor, negative_samples: bool = False
    ) -> float:
        if not negative_samples:
            all_z1 = self.all_gather(z1, sync_grads=True).flatten(0, 1)
            all_z2 = self.all_gather(z2, sync_grads=True).flatten(0, 1)
            return self.loss_unpaired(all_z1, all_z2)
        if self.world_size > 1:
            all_z1 = self.all_gather(z1, sync_grads=True).flatten(0, 1)
            all_z2 = self.all_gather(z2, sync_grads=True)
            ...
            all_negative_samples = (
                torch.cat(
                    [
                        all_z2[rank][self.per_device_batch_size :]
            ...
            # store_embeddings["negative_samples"] = central_modality_embedding_projected[batch_size:].reshape(
            #     batch_size, -1, central_modality_embedding_projected.shape[-1]
            # ).contiguous()
            # store_embeddings[self.central_modality] = central_modality_embedding_projected[:batch_size]
            # import pdb; pdb.set_trace()
            # shape z1 (World_Size, Batch_Size, Embedding_Size)
            # shape z2 (World_Size, Batch_Size, Embedding_Size)
            # shape negative_samples (World_Size, Batch_Size, Num_Negative_Samples, Embedding_Size)
            if torch.isnan(all_z1).any() or torch.isnan(all_z2).any():
                logger.error("Positive samples are nan.")
            unpaired_loss = self.loss_unpaired(all_z1, all_z2)
            paired_loss = self.loss(all_z1, all_z2, all_negative_samples) * self.beta(
                self.current_epoch
            )
            # log the unpaired and paired loss
            self.log("unpaired_loss", unpaired_loss)
            self.log("paired_loss", paired_loss)
            return unpaired_loss + paired_loss
        else:
            return self.loss(z1, z2, negative_samples)

    def forward(self, batch, negative_samples=True) -> dict:
        if isinstance(batch, tuple) and isinstance(batch[0], Data):
            dict_input_data = self._treat_graph_batch(batch[0])
            forward_pass = self.model(dict_input_data, negative_samples)
        else:
            forward_pass = self.model(batch, negative_samples)
        return forward_pass
    ...
    def _multimodal_loss(
        ...
        return central_to_modality_loss

    def validation_step(self, batch: dict) -> Tensor:
        embeddings_dict = self.forward(batch, negative_samples=False)
        return self._multimodal_loss(embeddings_dict, "valid")
