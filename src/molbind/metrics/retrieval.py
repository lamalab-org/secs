    ...
    for _, modality_1 in enumerate(all_modalities):
        ...
        for _, modality_2 in enumerate(all_modalities):
            if (
                ...
                and embeddings[modality_1].size() >= embeddings[modality_2].size()
            ):
                ...
                embed_mod1 = (
                    indices[[modality_1, modality_2]]
                    .dropna(subset=modality_1)
                    .reset_index()
                    .dropna(subset=modality_2)
                )
                embed_mod2 = (
                    indices[[modality_1, modality_2]]
                    .dropna(subset=modality_2)
                ...
                index_central_mod = (
                    indices[[modality_1, modality_2]].dropna().index.to_list()
                )
                central_modality_data = indices[central_modality][
                    index_central_mod
                ].to_list()
                logger.info(f"{modality_1} {modality_2} retrieval")
                logger.info(f"{len(central_modality_data)}")
                collection.add(
                    embeddings=embeddings[modality_1]
                    .detach()
                    .cpu()
                    .numpy()[index_embed_mod1],
                    documents=central_modality_data,
                    ids=central_modality_data,
                )
                modalities_retrieval[modality_2] = collection.query(
                    query_embeddings=embeddings[modality_2]
                    .detach()
                    .cpu()
                    .numpy()[index_embed_mod2],
                    n_results=max(top_k),
                )
